...
bool TestArea( CNavNode *node, int width, int height )
{
	...
	for( int y=0; y<height; y++ )
	{
		...
		for( int x=0; x<width; x++ )
		{
			...
			if (width > 1 || height > 1)
			{
				float dist = fabs(DotProduct( *horizNode->GetPosition(), normal ) + d);
		...
		if (width > 1 || height > 1)
		{
			float dist = fabs(DotProduct( *vertNode->GetPosition(), normal ) + d);
			if (dist > offPlaneTolerance)
				return false;
		}					
	}
...
bool IsSpotOccupied( CBaseEntity *me, const Vector *pos )
{
	...
	// (CS also checked for hostages here - TF has none)
...
bool IsCrossingLineOfFire( const Vector &start, const Vector &finish, CBaseEntity *ignore, int ignoreTeam  )
{
	for ( int p=1; p <= gpGlobals->maxClients; ++p )
	{
		...
		if (ignoreTeam && BotTeamID( player ) == ignoreTeam)
			continue;
...
int CNavArea::GetPlayerCount( int teamID, CBasePlayer *ignore ) const
{
	...
	for( int i=1; i<=gpGlobals->maxClients; ++i )
	{
		...
		if (teamID == 0 || BotTeamID( player ) == teamID)
	...
	for( i=0; i<NUM_DIRECTIONS; ++i )
	{
		...
		for( int a=0; a<count; ++a )
		{
			...
			if (isPlaceMode)
			{
				if (adj->GetPlace() == 0)
					adj->Draw( 50, 0, 0, 3 );
				else if (adj->GetPlace() != GetNavPlace())
					adj->Draw( 0, 0, 200, 3 );
				else
					adj->Draw( 0, 255, 0, 3 );
			}
				...
				switch( cmd )
				{
					...
					case EDIT_SPLIT:
						if (area->SplitEdit( splitAlongX, splitEdge ))
							// was CS's "weapons/knife_hitwall1.wav" - use a sound that ships with HL
						EMIT_SOUND_DYN( ENT(UTIL_GetLocalPlayer()->pev), CHAN_ITEM, "buttons/latchunlocked2.wav", 1, ATTN_NORM, 0, 100 );
						else
					...
					case EDIT_WARP_TO_MARK:
						if (markedArea)
						{
							...
							if ( pLocalPlayer && pLocalPlayer->pev->iuser1 == OBS_ROAMING )
							{
...
// (TheNavAreaGrid singleton is defined at the top of this file)
