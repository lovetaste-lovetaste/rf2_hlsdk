...
// How many extra cost-units to add per unit of height above the soft limit
// At 12.0 a step 50 units above the soft limit costs +600 on top of distance
...
// TODO: PORT THIS FUNCTION OVER
/*
	static float PathHelper_GetHeightDelta(int iFromArea, int iToArea)
	{
		int dir = PathHelper_FindConnDir(iFromArea, iToArea);
		if (dir != -1)
		{
			float flMyEdge[3], flOtherEdge[3], flHW;
			NavMeshArea_ComputePortal(iFromArea, iToArea, dir, flMyEdge, flHW);
			NavMeshArea_ComputePortal(iToArea, iFromArea, PathHelper_OppositeDir(dir), flOtherEdge, flHW);
			return flOtherEdge[2] - flMyEdge[2];
		}
		float flFrom[3], flTo[3];
		NavMeshArea_GetCenter(iFromArea, flFrom);
		NavMeshArea_GetCenter(iToArea,   flTo);
		return flTo[2] - flFrom[2];
	}
*/
...
public:
	float operator() ( CNavArea *area, CNavArea *fromArea, const CNavLadder *ladder )
	{
		...
		else
		{
			...
			float up = area->GetCenter()->z - fromArea->GetCenter()->z;
