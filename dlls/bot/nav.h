...
// NOTE: TheNavAreaList/TheNavAreaGrid externs live in nav_area.h, where their
// types are declared. Having them here broke files that include nav.h alone.
...
{
	if (fabs( AngleDifference( a, b ) ) < tolerance)
		return true;
...
inline bool IsWalkableTraceLineClear( Vector &from, Vector &to, unsigned int flags = 0 )
{
	...
	while(true)
	{
		...
		if (result.flFraction != 1.0f && IsEntityWalkable( VARS( result.pHit ), flags ))
		{
			...
			Vector dir = to - from;
			dir.NormalizeInPlace();	// Normalize() returns a copy in this SDK - must normalize in place
