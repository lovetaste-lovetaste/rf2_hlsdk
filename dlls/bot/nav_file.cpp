...
#include <io.h>
#ifndef MAX_OSPATH
#define MAX_OSPATH 260		// matches the definition in bot.h (from quakedef.h)
#endif
...
// TheBotPhrases is now the small standalone place-name registry in
// bot_phrases.h (the abstraction the old @todo here asked for), so the
// CS-specific cs_bot.h/cs_bot_manager.h includes are gone.
#include "shared_util.h"	// SharedParse()/SharedGetToken()
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return the size of the given file in bytes, or 0 if it can't be read.
 * This engine build has no pfnGetFileSize, so we load and immediately free
 * the file through the engine's filesystem to measure it (handles search
 * paths correctly, and only happens on nav save/load).
 */
static unsigned int GetFileSizeViaEngine( const char *filename )
{
	int length = 0;
	byte *data = (byte *)LOAD_FILE_FOR_ME( const_cast<char *>( filename ), &length );
...
bool SaveNavigationMap( const char *filename )
{
	...
	// Store the NAV file.
	//
	// The raw file IO below knows nothing about the engine's search paths,
	// and the process working directory is the engine root (not the mod
	// dir), so "maps/foo.nav" alone would land next to hl.exe - where the
	// engine filesystem can never load it back. Resolve the path against
	// the game directory instead. GET_GAME_DIR returns either "rooster" or
	// an absolute path depending on engine version; both work here.
	//
...
#ifdef WIN32
	// _O_TRUNC matters: without it, re-saving a smaller mesh over a larger
	// old file leaves stale bytes at the end, corrupting the nav file.
	int fd = _open( fullPath, _O_BINARY | _O_CREAT | _O_TRUNC | _O_WRONLY, _S_IREAD | _S_IWRITE );
#else
#define _write write
	int fd = creat( fullPath, S_IRUSR | S_IWUSR | S_IRGRP );
#endif
	...
	unsigned int bspSize = GetFileSizeViaEngine( bspFilename );
...
void LoadLocationFile( const char *filename )
{
	...
	if (dot)
	{
		...
		if (locData)
		{
			...
			// read directory (parsed with SharedParse - this file predates MP_COM_*)
			locData = SharedParse( locData );
			int dirSize = atoi( SharedGetToken() );
			...
			if (dirSize)
			{
				...
				while(true)
				{
					...
					areaID = atoi( SharedGetToken() );

					locData = SharedParse( locData );
					locDirIndex = atoi( SharedGetToken() );
